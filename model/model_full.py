...
from model.dataloader import adjacency_matrix, get_node_positions, get_edge_features
...
class Model(tf.keras.Model):
    ...
    def call(self, grid_static_feature):
        ...
        for gat_layer in self.gat_layers:
            self.g.ndata['embedding'] = gat_layer(self.g, self.g.ndata['embedding'], self.edge_embeddings)
    ...
    def compute_edge_embeddings(self, node_embeddings):
        ...
        edge_embeddings = tf.concat([src_emb, dst_emb, self.edge_embeddings], axis=-1)  # [e, 2d+d]
...
class GraphAttentionLayer(tf.keras.layers.Layer):
    ...
    def call(self, g, node_embeddings, edge_embeddings):
        """
        Forward pass for the GAT layer.
        """
        # Compute outputs from each attention head
        head_outputs = [head(g, node_embeddings, edge_embeddings) for head in self.attention_heads]
...
class AttentionHead(tf.keras.layers.Layer):
    ...
    def __init__(self, input_dim, output_dim, head_idx, layer_idx, **kwargs):
        ...
        self.W_e = self.add_weight(
            shape=(MODEL['output'][-1], MODEL['output'][-1]), 
            initializer='glorot_uniform', 
            trainable=True, 
            name=f'attention_W_e_{layer_idx}_{head_idx}'
        )
        ...
        self.a_edge = self.add_weight(
            shape=(MODEL['output'][-1], 1), 
            initializer='glorot_uniform', 
            trainable=True, 
            name=f'attention_a_edge_{layer_idx}_{head_idx}'
        )
    ...
    def call(self, g, node_embeddings, edge_embeddings):
        ...
        edge_embeddings_transformed = tf.matmul(edge_embeddings, self.W_e)  # [e, d] * [d, d]

        # Assign transformed node embeddings to graph
        g.ndata['embedding'] = node_embeddings_transformed
        g.edata['edge_embedding'] = edge_embeddings_transformed
    ...
    def message_func(self, edges):
        ...
        h_edge = tf.matmul(edges.data['edge_embedding'], self.a_edge)
        e = tf.nn.leaky_relu(h_src + h_dst + h_edge)
